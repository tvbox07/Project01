...
def create_value_price(stock_no, interest_rate):
    ...
    try:
        ...
        # print("자기자본총계(지배) : {0:,}".format(owner_capital, ','))
    except:
        owner_capital = 0
    ...
    try:
        ...
        # print("상장주식수 : {0:,} ".format(stock_count,','))
    except:
        stock_count = 1
    ...
    browser.quit()
    return owner_capital, roe_3yr, stock_count, round(stock_current_price), round(stock_value_price), url, evevitda
...
if __name__=="__main__":
    ...
    g = open("D:\Coding\PythonWorkspace02\webscrapping\stock_scrapping\stock_list02.csv", "r", encoding="utf-8-sig")
    reader = csv.reader(g)
    data = list(reader)
    # print("csv length : ", len(data))

    # csv 에 자료 저장
    f = open("D:\Coding\PythonWorkspace02\webscrapping\stock_scrapping\stock_list02_result.csv", "w", encoding="utf-8-sig", newline="") #한글이 깨지지 않게 저장하기 utf-8-sig
    #   # 기존 csv에 추가해서 자료 저장
    ...
    writer = csv.writer(f)
    title = "stock_no, com_name, owner_capital, roe_3yr, stock_count, stock_current_price, stock_value_price,expected_rise_percent, url, evevitda".split(",")
    #["N", "종목명", "현재가",....] 리스트 형태로들어감
    # print(type(title))
    writer.writerow(title)

    j = 0
    # for line in data[:5]: # 5개까지만 출력 테스트 시
    for line in data: # 전체 출력
        ...
        owner_capital, roe_3yr, stock_count, stock_current_price, stock_value_price, url, evevitda = create_value_price(stock_no, interest_rate)
        # print("stock_current_price : {0:,}" .format(stock_current_price)) 
        # print("stock_value_price : {0:,}" .format(stock_current_price))  

        # (적정주가 - 현재주가)/ 현재주가 * 100 = 상승여력  
        expected_rise_percent = ((stock_value_price - stock_current_price)/stock_current_price) * 100
        # print("상승하락 여력 : ", expected_rise_percent)
        alist = [stock_no, com_name, owner_capital, roe_3yr, stock_count, stock_current_price, stock_value_price,expected_rise_percent, url, evevitda]
